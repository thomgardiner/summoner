...
pub fn render(events: &str, now: u64) -> (String, bool) {
        ...
        match event["event"].as_str().unwrap_or_default() {
            ...
            name => {
                ...
                match name {
                    ...
                    "order_dispatched" => row.phase = "executing".into(),
                    "order_exec_done" => {
    ...
    out.push_str(&format!(
        "{:<20} {:<10} {:<3} {:<22} {:>8} {:>7}  {}\n",
        "ORDER", "EXECUTOR", "TRY", "PHASE", "ELAPSED", "TOKENS", "DETAIL"
    ));
    let mut total_tokens = 0u64;
    for (id, row) in &rows {
        let elapsed = row.ended.unwrap_or(now).saturating_sub(row.started);
...
{"ts":102,"event":"order_dispatched","id":"auth"}
...
{"ts":150,"event":"order_finished","id":"auth","outcome":"approved","usage_tokens":900}
...
"#;
        ...
        // Attempt column reflects the revision.
