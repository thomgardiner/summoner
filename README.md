...
`summoner watch` renders a live terminal board over the latest run's events (or `summoner watch <run-id>`): one row per order with phase, attempt, elapsed time, and token usage, exiting when the run finishes.
...
Budgets are enforced two ways. `run_token_budget = N` (or `SUMMONER_RUN_TOKEN_BUDGET`) is a run-wide breaker: once scraped usage crosses it, the remaining queue lands as `skipped`. A per-order `max_tokens` stops revisions for an order that overspent and calls the overage out in its report entry. Both act between dispatches — usage is only known after an executor exits — so one in-flight order can overshoot before the breaker sees it; the grove deadline remains the hard stop for a runaway process.
